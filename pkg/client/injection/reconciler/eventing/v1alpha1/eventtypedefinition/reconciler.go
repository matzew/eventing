...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1alpha1.EventTypeDefinition, desired *v1alpha1.EventTypeDefinition) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.EventingV1alpha1().EventTypeDefinitions(desired.Namespace)
		...
		updater := r.Client.EventingV1alpha1().EventTypeDefinitions(existing.Namespace)
